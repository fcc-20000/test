...
import cn.bdqn.warehousemanager.warehouse.service.outwarehouse.OutWarehouseService;
...
public class OutWarehouseController {
    ...
    @Autowired
    OutWarehouseService warehousingService;//入库表单
    @Autowired
...
//    @ResponseBody
    public ModelAndView getWarhousing(OutWarehouseVo warehousingVo) {
        ...
        Page page = new Page(warehousingVo.getPageNo(), 1, total);
        List<Audit> auditList = auditService.getAuditAll();
        List<WarehousingType> warehousingTypeList = warehousingTypeService.getOutWarehousingTypeAll();
        warehousingVo.setPageNo(page.getBeginPos());
        warehousingVo.setPageSize(page.getPageSize());
        List<OutWarehouseVo> list = warehousingService.getAllWarehousing(warehousingVo);
        page.setRows(list);
        ...
        modelAndView.setViewName("outWarehousing");
        return modelAndView;
    }
    ...
    public ModelAndView getDetailed(Integer id) {
        ...
        OutWarehouseVo warehousingVo = warehousingService.getWarehousingVoById(id);
        modelAndView.setViewName("OutstorageDetails");
    ...
    public ModelAndView addWarehousingHtml() {
        ...
        modelAndView.setViewName("addOutWarehousing");
        return modelAndView;
    }
...
//        warehousingProductsVo.setFounder(founder);
        ...
        if (result > 0) {
            return "redirect:/api/house/getWarehousing";
        }
    ...
    public ModelAndView updateWarehousingHtml(Integer id) {
        ...
        modelAndView.addObject("warehousingVo", warehousingVo);
        modelAndView.setViewName("updateOutWarehousing");
        return modelAndView;
    }
    ...
    public String updateWarehousingVo(UpdateWarehousingVo updateWarehousingVo) {
        ...
        return "redirect:/api/house/updateWarehousingVo";
    }
    ...
    public ModelAndView updateReviewHtml(Integer id) {
        ...
        List<WarehousingProducts> warehousingProductsList = warehousingproductsService.getWarehousingProductsById(id);
        OutWarehouseVo warehousingVo = warehousingService.getWarehousingVoById(id);
        modelAndView.setViewName("updateOutReview");
        modelAndView.addObject("warehousingProductsList", warehousingProductsList);
        modelAndView.addObject("warehousingVo", warehousingVo);
        return modelAndView;
    }
    ...
    public String updateReview(Integer id, Integer aid) {
        ...
        if (result > 0) {
            ...
            return "redirect:/api/house/updateReviewHtml?id=" + id;
        }
    }
